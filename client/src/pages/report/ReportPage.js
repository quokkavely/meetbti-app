    ...
    return (
        <div className="app">
            ...
            <div className="backoffice-radio">
                ...
                onClick={() => {setSelectedButton('신고'); setIsLoading(true)}}>신고 목록</button>
                <button className={'backoffice-radio-button' + (selectedButton === '이미지' ? '-selected' : '')}
                onClick={() => {setSelectedButton('이미지'); setIsLoading(true)}}>이미지게임</button>
                <button className={'backoffice-radio-button' + (selectedButton === '밸런스' ? '-selected' : '')}
                onClick={() => {setSelectedButton('밸런스'); setIsLoading(true)}}>밸런스게임</button>
            </div>
            {/* <ReportHeader button='신고 처리'></ReportHeader> */}
            {
            reports.data.length === 0 ? <div>{`요청된 ${noContentMessage} 없어요`}</div> : 
            isLoading ? <div></div> : <ReportPageContent reports={reports} selectedButton={selectedButton}></ReportPageContent>
            }
